...
class TestRectangle(unittest.TestCase):
    ...
    def test_load_from_file_nonexistent_file(self):
        ...
        self.assertEqual(Rectangle.load_from_file(filename), [])

    def test_load_from_file_existing_file(self):
        """Test of Rectangle.load_from_file() when file exists"""
        filename = "existing_file.json"
        rectangles = [Rectangle(1, 2, 3, 4, 89), Rectangle(2, 3, 4, 5, 90)]
        Rectangle.save_to_file(rectangles, filename)
        self.assertTrue(os.path.exists(filename))
        loaded_rectangles = Rectangle.load_from_file(filename)
        for i, rectangle in enumerate(loaded_rectangles):
            self.assertEqual(rectangle.to_dictionary(), rectangles[i].to_dictionary())
        os.remove(filename)
